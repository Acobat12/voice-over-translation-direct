...
## Публикация локальных видео через Caddy и Cloudflare Tunnel

Для публикации и перевода локальных видео можно использовать вспомогательные скрипты `start_public_video.ps1` и `start_public_video.bat`.

Теперь локальный запуск работает так:

- `.mp4`-файлы раздаются напрямую как обычные файлы
- если исходное видео **не** в формате `.mp4` (`.mkv`, `.avi`, `.mov`, `.m4v`, `.webm` и т.д.), скрипт автоматически конвертирует его в `.mp4`
- конвертированные файлы сохраняются в папку `.public-video-cache`
- при следующем запуске повторная конвертация **не выполняется**, если для этого видео уже есть готовый `.mp4` в `.public-video-cache`

Скрипты больше не требуют Python. Вместо него используются:

- локальный статический сервер на **Caddy**
- публичный **Cloudflare Quick Tunnel**
- **FFmpeg** для одноразовой конвертации файлов, которые изначально не были `.mp4`

### Что делает скрипт

- проверяет наличие `caddy`, `cloudflared` и `ffmpeg`
- если их нет, скачивает portable-версии в папку `.tools`
- создаёт `player.html` и `playlist.json`
- ищет видеофайлы в текущей папке
- `.mp4` оставляет как есть
- остальные форматы конвертирует в `.mp4` и кладёт в `.public-video-cache`
- запускает локальный сервер на `http://127.0.0.1:8000`
- поднимает Cloudflare Quick Tunnel для этого сервера
- получает публичную ссылку вида `https://*.trycloudflare.com/player.html?list=/playlist.json`
- копирует ссылку в буфер обмена
- пытается открыть ссылку в браузере
- при завершении останавливает `caddy` и `cloudflared`

### Важно про папку `.tools`

Папка `.tools` содержит уже скачанные portable-утилиты (`caddy`, `cloudflared`, `ffmpeg`).
Её можно **целиком перенести** в другую папку, где лежат видео, чтобы при следующем запуске скрипт **не скачивал инструменты заново**.

Пример:

1. Один раз запустили скрипт в первой папке с видео
2. Получили папку `.tools`
3. Скопировали `.tools` в другую папку с видео
4. Запустили `start_public_video.bat` уже там
5. Скрипт сразу использует готовые инструменты из `.tools`

### Важно про папку `.public-video-cache`

Папка `.public-video-cache` хранит уже подготовленные `.mp4`-файлы для видео, которые изначально были в другом формате.

Это значит:

- первый запуск может занять больше времени, если есть `.mkv`, `.avi`, `.mov` и другие не-`.mp4` файлы
- последующие запуски будут быстрее, потому что скрипт использует уже готовые `.mp4` из `.public-video-cache`
- если готовый `.mp4` уже существует в `.public-video-cache`, повторная конвертация не выполняется

### Пошаговый запуск для новичков

1. Поместите `start_public_video.ps1` и `start_public_video.bat` в папку с видео
2. Дважды кликните по `start_public_video.bat` или запустите его из PowerShell:

```powershell
.\start_public_video.bat
```

3. Дождитесь, пока скрипт:
   - проверит и при необходимости скачает инструменты в `.tools`
   - создаст `player.html` и `playlist.json`
   - при необходимости сконвертирует не-`.mp4` видео в `.public-video-cache`
   - поднимет локальный сервер и Cloudflare Tunnel
4. После этого скрипт покажет публичную ссылку и скопирует её в буфер обмена
5. Откройте эту ссылку в браузере — откроется `player.html` со списком видео справа

### Если нужно сразу открыть конкретный файл

Можно запустить PowerShell-скрипт вручную и указать имя файла:
...
Если файл не `.mp4`, скрипт сначала подготовит для него `.mp4`-версию в `.public-video-cache`, а затем откроет плеер.

### Что создаётся рядом с видео

Скрипт создаёт и использует следующие файлы и папки:

- `player.html` — локальный плеер с очередью видео
- `playlist.json` — список видео для плеера
- `.tools` — portable-утилиты (`caddy`, `cloudflared`, `ffmpeg`)
- `.public-video-cache` — кэш уже сконвертированных `.mp4`

### Ограничения

- используется **Cloudflare Quick Tunnel**, поэтому публичный URL каждый раз новый
- при первом открытии DNS для `*.trycloudflare.com` может появиться не сразу
- скорость первого запуска зависит от количества видео и от того, нужно ли конвертировать файлы в `.mp4`
- при остановке скрипта завершаются и локальный сервер, и tunnel
- скрипт ищет видео в текущей папке, где лежат `start_public_video.ps1` и `start_public_video.bat`
- если не-`.mp4` видео уже было сконвертировано и готовый файл есть в `.public-video-cache`, повторная конвертация не выполняется
