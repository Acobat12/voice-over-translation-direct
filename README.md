...
Закадровый перевод видео теперь доступен не только в [YandexBrowser][yabrowser-link].

Форк содержит собственные изменения и развивается независимо от основной версии VOT. Некоторые функции, настройки и особенности работы могут отличаться от оригинального проекта.

Проект использует технологии **[Yandex.Translate][yatranslate-link]** и развивается как независимый форк VOT.

## Известные конфликты расширений

> [!WARNING]
> Наблюдался конфликт с расширением **YouTube Dubbing / YouTube中文配音** из Firefox Add-ons: при совместной работе могли появляться задержки и подвисания на YouTube, VK Видео и других сайтах.

## Публикация локальных видео

В этом форке локальные видео можно открывать по временной публичной ссылке через приложение **`PublicVideoLauncher`**.

> [!IMPORTANT]
> Для перевода локальных видео необходим **`vot.user.js`**.  
> Без него `PublicVideoLauncher` сможет открыть локальное видео по публичной ссылке, но сам перевод работать не будет.
...
- Локальный перевод видео через PublicVideoLauncher (Windows, macOS и Linux)
- Публикация локальных видео по временной публичной ссылке
- Поддержка нескольких туннельных провайдеров (Cloudflare Tunnel, Cloudflare Name, localhost.run, Tailscale Funnel, zrok, xTunnel)
...
- **Google Drive:** перевод запускается непосредственно из плеера Google Drive через кнопку VOT.
