...
## Публикация локальных видео через Python и Cloudflare Tunnel

Для локальных `.mp4` / `.webm` / других видеофайлов можно использовать вспомогательные скрипты `start_public_video.ps1` и `start_public_video.bat`.

Что делает скрипт:
- проверяет наличие `python` и `cloudflared`
- запускает локальный HTTP-сервер на `127.0.0.1:8000`
...
- пытается открыть ссылку в Chrome с DNS override
- при завершении останавливает `python` и `cloudflared`

### Как запустить

1. Убедитесь, что установлен Python
2. Убедитесь, что установлен `cloudflared`
3. Поместите `start_public_video.ps1` и `start_public_video.bat` в папку с видео
4. Запустите:
...
- скрипт поднимет локальный сервер на `http://127.0.0.1:8000`
- создаст публичную ссылку `https://*.trycloudflare.com`
- скопирует её в буфер обмена
- попытается открыть ссылку в Chrome

### Ограничения

- используется **Cloudflare Quick Tunnel**, поэтому публичный URL каждый раз новый
- если hostname `trycloudflare.com` ещё не успел распространиться по DNS, ссылка может открыться не сразу во всех браузерах
- наиболее стабильно открытие работает через Chrome с DNS override
- при остановке скрипта tunnel и локальный HTTP-сервер завершаются
