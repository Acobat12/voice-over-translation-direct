...
Для локальных видеофайлов можно использовать вспомогательные скрипты `start_public_video.ps1` и `start_public_video.bat`. Для локального перевода поддерживаются только файлы в формате `.mp4`.

Скрипты больше не требуют Python. Вместо него используется локальный статический сервер на **Caddy** и публичный **Cloudflare Quick Tunnel**.

Что делает скрипт:

- проверяет наличие `caddy` и `cloudflared`
- если их нет, скачивает portable-версии в папку `.tools`
- создаёт `player.html` для открытия отдельного видео
- создаёт `index.html` со списком видеофайлов в текущей папке
- запускает локальный сервер на `http://127.0.0.1:8000`
- поднимает Cloudflare Quick Tunnel для этого сервера
- получает публичную ссылку вида `https://*.trycloudflare.com`
...
- скрипт создаст `index.html` и `player.html` рядом с вашими видео
- локально будет доступна папка по адресу `http://127.0.0.1:8000/`
- будет создана публичная ссылка `https://*.trycloudflare.com`
- ссылка будет скопирована в буфер обмена
- если открывается вся папка, используйте корневой URL туннеля
- для отдельного видео используется ссылка вида `player.html?src=/video.mp4`

### Открытие одного файла напрямую

Если нужно сразу открыть конкретный файл, можно запустить PowerShell-скрипт вручную:
...
- скорость старта зависит не только от туннеля, но и от того, как браузер начинает читать сам видеофайл
- при остановке скрипта завершаются и локальный сервер, и tunnel
- `index.html` показывает видео только из текущей папки скрипта
- для локального перевода поддерживаются только `.mp4`-файлы
