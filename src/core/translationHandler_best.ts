...
const YOUTUBE_SERVER_POLL_RETRY_DELAY_MS = 10000;
// Diagnostic: after a successful initial YouTube WAITING response, re-check the
// accepted request shape three times before giving up. This intentionally keeps
// firstRequest=true for those poll attempts so we can compare against the first
// request that created the translation task.
const MAX_YOUTUBE_SERVER_POLL_ERROR_RETRIES = 3;
...
export class VOTTranslationHandler {
  ...
  private youtubeInitialRequestAccepted = false;
  ...
  resetTranslationRequestState(reason?: unknown): void {
    ...
    this.youtubeInitialRequestAccepted = false;
  ...
  private shouldRetryYouTubeProcessingError(error: unknown): boolean {
    ...
    const combinedMessage = `${message} ${serverMessage}`.toLowerCase();

    return (
      message === "Failed to request video translation" ||
      message === "Yandex couldn't translate video" ||
      combinedMessage.includes("возникла ошибка") ||
      combinedMessage.includes("попробуйте позже") ||
  ...
  private rememberYouTubeServerPollState(
    ...
    requestVideoData: VideoData | undefined,
    requestWasFirst: boolean,
  ): void {
    if (!this.isYouTubeTranslationRequest(requestVideoData)) {
      return;
    }

    // AUDIO_REQUESTED is not proof that a server-side translation task entered
    // the normal processing queue. Only WAITING/LONG_WAITING may enable polling.
    if (
    ...
    if (requestWasFirst) {
      this.youtubeInitialRequestAccepted = true;
      this.youtubeServerPollActive = true;
      this.youtubeServerPollErrorCount = 0;
      console.log(
        "[VOT][youtube/first-request-check] initial request accepted",
        {
          status: response.status,
          remainingTime: response.remainingTime,
          translationId:
            response.translationId === undefined ||
            response.translationId === null
              ? undefined
              : String(response.translationId),
        },
      );
    } else if (
      !this.youtubeInitialRequestAccepted ||
      !this.youtubeServerPollActive
    ) {
      // Never create a poll state from a later/ambiguous response.
      return;
    }
  ...
  > {
    ...
    try {
      ...
      let successfulRequestWasFirst = false;

      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          const computedFirstRequest = !this.translationRequestStarted;
          const isYouTubeRequestForCall =
            this.isYouTubeTranslationRequest(requestVideoData);
          // Controlled diagnostic: once the *real* first request was accepted as
          // WAITING/LONG_WAITING, keep the same firstRequest flag for the three
          // verification polls.
          const effectiveFirstRequest =
            isYouTubeRequestForCall && this.youtubeInitialRequestAccepted
              ? true
              : computedFirstRequest;
          ...
          const youtubeFailedAudioSignalAlreadySent =
            isYouTubeRequestForCall &&
            this.youtubeFailedAudioSignalUrl === youtubeFailedAudioSignalUrl;
          const allowFailedAudioSignal = isYouTubeRequestForCall
            ? !youtubeFailedAudioSignalAlreadySent
            : shouldSendFailedAudio;
          // Keep the full request shape on YouTube polling. Previously polling
          // replaced extraOpts with {}, so the console's firstRequest value did
          // not necessarily match what the protobuf encoder actually received.
          const extraOpts = {
            useLivelyVoice,
            forceSourceLang: requestLang !== "auto",
            firstRequest: effectiveFirstRequest,
            videoTitle:
          ...
          if (isYouTubeRequestForCall && allowFailedAudioSignal) {
            this.youtubeFailedAudioSignalUrl = youtubeFailedAudioSignalUrl;
          }
          ...
          console.log("[VOT][youtube/request-snapshot]", {
            ...
            computedFirstRequest,
            effectiveFirstRequest,
            initialRequestAccepted: this.youtubeInitialRequestAccepted,
            forceSourceLang: requestLang !== "auto",
            useLivelyVoice,
          ...
          successfulRequestWasFirst = computedFirstRequest;
      ...
      if (isYouTubeIntermediateStatus) {
        this.rememberYouTubeServerPollState(
          res,
          requestVideoData,
          successfulRequestWasFirst,
        );
      }
      ...
      if (
        ...
        console.warn(
          "[VOT][youtube/first-request-check] AUDIO_REQUESTED is not a pollable accepted first request; stopping",
          {
            translationId: res.translationId,
            remainingTime: res.remainingTime,
            requestUrl: requestVideoData.url,
      ...
      // Polling must not wait for UI text updates. On some pages the UI branch
      // can perform async localization and stall the next /translate request.
      if (
        res.status === VideoTranslationStatus.WAITING ||
        res.status === VideoTranslationStatus.LONG_WAITING
      ) {
        ...
        return this.scheduleRetry(
          ...
          isYouTubeRequest ? YOUTUBE_SERVER_POLL_RETRY_DELAY_MS : 5000,
          signal,
        );
      }
      ...
      if (
        ...
        console.warn(
          "[VOT][youtube/server-poll] transient poll failure; retrying without resetting translation state",
          {
            ...
            retryDelayMs: YOUTUBE_SERVER_POLL_RETRY_DELAY_MS,
            videoId: videoData.videoId,
            activeTranslationUrl: this.activeTranslationUrl,
        ...
        return this.scheduleRetry(
          ...
          YOUTUBE_SERVER_POLL_RETRY_DELAY_MS,
          signal,
        );
      }
