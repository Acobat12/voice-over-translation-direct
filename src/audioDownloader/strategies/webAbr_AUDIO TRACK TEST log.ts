...
import { prepareYouTubePlayer } from "./ytPlayerSolver";
...
function selectWebEmbeddedAudioFormat(formats) {
	...
	// TEMP DIAGNOSTIC: log every audio-only candidate and the exact format
	// selected by webAbr. This is useful for YouTube multi-audio videos where
	// the same base itag can exist for many different language tracks.
		...
		JSON.stringify(
			{
				...
				selectionMode:
					defaultAudioOnly.length > 0 ? "audioIsDefault" : "legacy-fallback",
				defaultCandidates: defaultAudioOnly.map(describeAudioFormat),
